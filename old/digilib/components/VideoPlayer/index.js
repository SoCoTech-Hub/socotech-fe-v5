...
import  getConnectionSpeed  from '@/snippets/digilib/getConnectionSpeed'
...
const VideoPlayer = ({ src }) => {
	...
	useEffect(async () => {
		// Load 'videojs-quality-selector' library dynamically
		const videojsQualitySelector = await import(
			'@silvermine/videojs-quality-selector'
		)

		// Load 'quality-selector.css' dynamically
		await import(
			'@silvermine/videojs-quality-selector/dist/css/quality-selector.css'
		)

		// Use videojs from the dynamic import
		const videojs = videojsQualitySelector.default || videojsQualitySelector
