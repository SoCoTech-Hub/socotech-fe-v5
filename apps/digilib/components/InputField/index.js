...
const InputField = ({
	...
	// Convert `useDebounce` to only affect numeric input types
	const debouncedValue = useDebounce({
		value: inputValue,
		delay: type === 'number' ? 1000 : 0
	})
	...
	useEffect(() => {
		if (type === 'number' && debouncedValue !== value) {
			onChange({
		...
		>
			<div className={`my-1 rounded-lg shadow-md ${icon ? 'relative' : ''}`}>
				...
				<input
					...
					className={`w-full text-themeColorMain bg-compBg bg-opacity-20 rounded-lg py-2 block border-2 ring-inset ring-white placeholder-gray-500 focus:ring-2 focus:ring-inset focus:ring-themeColorMain mobile:text-sm mobile:leading-6 ${
						icon
							? type == 'password'
								? 'py-1.5 pl-7 pr-12'
								: 'py-1.5 pl-7'
