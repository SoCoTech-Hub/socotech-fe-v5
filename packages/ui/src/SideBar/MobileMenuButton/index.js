import Link from "next/link";
import { NotificationDot } from "@/components/NotificationDot";
import { Icon1 } from "@/context/constants";

const MobileMenuButton = ({
  icon,
  color = "",
  title = "Item",
  link,
  doWhat,
  // iconColor,
  textColor,
  hasNotification,
  isActive,
  animate = false,
}) => {
  const IsLocal = ({ children }) => {
    return link.startsWith("http") ? (
      <a onClick={doWhat} href={link}>
        {children}
      </a>
    ) : (
      <Link href={link} passHref>
        <a onClick={doWhat}>{children}</a>
      </Link>
    );
  };

  return (
    <div className="cursor-pointer">
      <IsLocal>
        <div className="-mx-1 flex h-20 items-center align-middle">
          <div className="my-2 flex w-24 flex-wrap px-0.5">
            <div className="justify-content-center flex w-full">
              {icon && (
                <div
                  className={`h-12 w-12 rounded-full p-0.5 ${
                    hasNotification ? "" : "overflow-hidden"
                  } ${color} ${
                    isActive
                      ? "bg-themeColorMain text-white"
                      : "bg-compBg text-themeColorSecondary"
                  }`}
                >
                  <div className="" style={{ color: Icon1 }}>
                    {icon}
                    {hasNotification ? (
                      <div className="-mx-1 my-[0.6]">
                        <NotificationDot active={isActive} />
                      </div>
                    ) : (
                      <></>
                    )}
                  </div>
                </div>
              )}
            </div>
            <div className="justify-content-center flex w-full">
              <div
                className={`${
                  isActive ? "font-bold" : ""
                } mt-1 text-center text-xs ${textColor} ${
                  animate ? "animate-bounce" : ""
                }`}
              >
                {title}
              </div>
            </div>
          </div>
        </div>
      </IsLocal>
    </div>
  );
};

export default MobileMenuButton;
